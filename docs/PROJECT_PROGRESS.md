...
was a genuine bug, and `custom13` just needed a Target Pose picked (a
data gap, not code).
...
**SHIPPED 2026-09-20 (later same day, 17th follow-up round,
verification gap disclosed) -- "Show in Mobile/Landscape" is now
universal (every control, not just opted-in ones) + groups auto-hide
when empty**, referencing CLICKO's own actual implementation (after 2
name corrections: "DickoClicko" -> "ClickoDicko" -> the real project,
`J:\CLAUDE\PROJECTS\CLICKO`). CLICKO's own pattern: no per-group
persisted flag at all -- group visibility is COMPUTED from whether it
still has any visible child, refreshed every pass. New
`isDynamicDeviceCtrl(ctrl)` (devPanel.js) makes every control eligible
by default (unless `perDevice`, an excluded type, or explicitly opted
out) -- covers every group that never had this at all before (Field
Layout, Camera, Lighting, Toon Shading, Outline, Background, Debug,
Pose, etc.), default checked/visible everywhere so nothing changes
until someone actively unchecks something. New
`refreshEmptyGroupVisibility()` hides a group (via CSS class, not
inline style) the instant none of its children are visible on Mobile/
Landscape -- so unchecking a group's own cascade checkbox now hides the
WHOLE group, not just an empty shell. **Verification gap:** hit this
sandbox's own browser tool connection flakiness across 5+ attempts,
but confirmed via 2 separate `curl` checks that both changed files
deliver completely and correctly outside the tool -- `node --check`
passes; confidence rests on the refactor being behavior-preserving by
construction and directly mirroring CLICKO's own proven pattern.
**Genuinely needs a live check on your next reload**, given the scale
(every control in the panel is affected). See CHANGELOG.txt's matching
